...
class DropwizardPlugin implements Plugin<Project> {
	...
	void configureTestTask(Project project, String taskName) {
		...
		project.configure(project) {
			...
			task("${taskName}Launcher", dependsOn: [ "${taskName}Classes", "classes", "test" ]) << {
			...
			task("${taskName}", type: Test.class, dependsOn: "${taskName}Launcher") {
			...
			task("${taskName}Finalizer") << {
				...
				int rv = project."${taskName}Process"?.shutdown()
